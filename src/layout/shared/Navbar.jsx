...
export default function Navbar() {
	...
	return <div className="z-[9999]">
		<div className="relative flex items-center justify-between h-16 px-8 md:h-20 bg-secondary-2">
		...
		{/* Overlay to close dropdowns */}
			<div onClick={ () => { setOpenMenu(false); setOpenAvatar(false) } } className={`fixed ${(openMenu || openAvatar) ? "block" : "hidden"} top-0 left-0 w-screen h-screen z-[50]`}></div>
		
		{/* Logo */}
		<button>
		<Link to="/" className="flex items-center lg:gap-4 gap-2">
			<img className="hidden w-12 md:block lg:w-16" src="/logo.png" alt="Logo"/>
			<h1 className="text-4xl lg:text-6xl text-primary font-banger">Purrlinks</h1>
		</Link>
		</button>
				
		{/* Menu Dropdown */}
		<div className={`z-50 ${openMenu ? "block md:hidden" : "hidden"} my-4 absolute top-12 left-2 text-base list-none bg-neutral rounded shadow divide-y divide-gray-100`} >
			<ul className="py-1">
		...
		{/* User */}
		<div className="flex">
			<button onClick={ () => {setOpenAvatar(!openAvatar); setOpenMenu(false)} } className="w-8 h-8 rounded-full md:w-12 md:h-12 outline outline-black/5 hover:outline-black/10 transition ease-out duration-200 hover:-translate-y-px">
			...
			{/* Avatar Dropdown */}
			<div className={`z-50 ${openAvatar ? "block" : "hidden"} my-4 absolute top-12 md:top-16 right-2 text-base list-none bg-neutral rounded shadow divide-y divide-gray-100`} >
